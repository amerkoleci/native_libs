...
_ALIMER_GPU_EXTERN __declspec(dllimport) HMODULE __stdcall LoadLibraryA(LPCSTR);
_ALIMER_GPU_EXTERN __declspec(dllimport) FARPROC __stdcall GetProcAddress(HMODULE, LPCSTR);
#else
#include <dlfcn.h>
#endif
    ...
    void*/* pUserData*/)
{
    ...
    if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT)
    {
        ...
        {
            //state.config.callback("Vulkan - %s: %s", messageTypeStr, pCallbackData->pMessage);
        }
    }
    else if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
    {
        if (state.config.callback)
        {
            //Alimer_LogError("Vulkan - %s: %s", messageTypeStr, pCallbackData->pMessage);
        }
    }
